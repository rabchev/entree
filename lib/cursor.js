 ...
 * Options
 *
 *  - `skip` {number}, The number of documents to skip from the beginning of the matching result.
 *  - `limit` {number}, Limit the number of documents to return.
 *  - `sort` {object}, Controls the order that the query returns matching documents.
 *  - `map` {object}, Specifies which fields from the document should be returned.
 *
...
/**
 * Specifies which fields from the document should be returned.
 *
 * @example
 *  provider
 *      .select()
 ...
 *          // -> The documents in the array will have only name and age fields.
 *      });
 *
 * @param {Object} criteria - The criteria by which the result set will be sorted.
 ...
 *
 * @param {Function} callback This will be called after executing this method successfully. The first parameter will contain the Error object if an error occurred, or null otherwise. The second parameter will contain an array of objects as a result of the query.
 * @return {null}
 * @api public
 */
 ...
 *
 * @param {Function} callback this will be called for while iterating every document of the query result.
 * The first parameter will contain the Error object if an error occured, or null otherwise.
 * While the second parameter will contain the document.
 * @return {null}
 * @api public
 */
...
Cursor.prototype.update = function (data, callback) {
    this.__uprem(data, callback);
};

Cursor.prototype.delete = function (callback) {
    this.__uprem(null, callback);
};

Cursor.prototype.__uprem = function (data, callback) {
    var that = this;
        ...
        async.each(
            ...
            function (item, cb) {
                if (data) {
                    ...
                    that.provider.update(null, item, cb);
                } else {
                    that.provider.delete(null, item, cb);
                }
            },
            callback
        );
