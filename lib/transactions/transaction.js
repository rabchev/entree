            ...
            if (method.success) {
                fns.push(function (cb) {
                    if (method.name === "select") {
                        ...
                        var coll = trans.dataManager[coll.name];
                        switch (method.name) {
                        case "insert":
                            coll.delete(context, method.data, cb);
                            break;
                        case "upsert":
                            if (method.oldData) {
                                coll.update(context, method.oldData, cb);
                            } else {
                                coll.delete(context, method.data, cb);
                            }
                            break;
                        case "update":
                            ...
                            break;
                        case "delete":
                            coll.insert(context, method.oldData, cb);
                            break;
                        }
                    }
                });
...
Transaction.prototype.rollback = function (context, callback, commitErr) {
    ...
    this.transMan.store.update(this.toPersistent(), function (err) {
        ...
        var rbFns = getRollbacks(this, context);
    ...
    async.parallel(readFns, function (err) {
        ...
        that.transMan.store.upsert(that.toPersistent(), function (err) {
            ...
            async.series(commFns, function (err, res) {
                if (err) {
                    return that.rolleback(context, callback, err);
                }
