...
Manager.prototype.addCollection = function (name, connection, interceptors, schema, done) {
    ...
    function setProvider() {
        var conn        = that.connections[connection],
            provider;
        ...
        if (!conn) {
            return done(new Err("no such connection"));
        }

        provider = new (that.resolveProvider(conn.provider))(conn.options, schema);
...
Manager.prototype.removeProvider = Manager.prototype.removeCollection;
