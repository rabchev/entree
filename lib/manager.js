...
Manager.prototype.init = function (callback) {
    ...
    that.config.init(function (err) {
        ...
        if (that.model) {
            ...
            that.config.get(null, that.dataModelDoc || "dataModel", function (err, model) {
                if (err) {
                    // TODO: refactor it!
                    if (that.dataModelDoc) {
                        err = new Error("The specified configuration document doesn't exist.");
                    } else {
                        err = new Error("");
                    }
                    callback(err);
