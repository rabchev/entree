...
Manager.prototype.init = function (opts, callback) {
    ...
    if (opts.config) {
        if (!opts.config.modelDocument || !opts.config.provider || !opts.config.options || !opts.config.schema) {
            return callback(new Err("MISSING_CONF_PARAMS"));
        }
        dataModelDoc = opts.config.modelDocument;
        confProv = new (this.resolveProvider(opts.config.provider))(opts.config.options, opts.config.schema);
    } else if (!this.config) {
        dataModelDoc = "dataModel";
        confProv = new (this.resolveProvider("file-system"))({ connStr: path.join(process.cwd(), "data") }, { __collName: "config" });
    }

    if (opts.model) {
        //this.model = opts.model;
        initProviders(that, opts.model, callback);
    }
