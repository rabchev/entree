...
var TransactionManager  = require("./transactions/transaction-manager"),
...
function mergeReplicas(man, source) {
    ...
    // TODO: Implement merge replica set configuration. [2014-01-12]
}

function mergeShards(man, source) {
    // TODO: Implement merge shard configurations. [2014-01-12]
}
...
var addConnection = function (name, provider, opts) {
    ...
    addReplicaSet = function (name, collections, opts, done) {
        ...
        var replica = new Replica(this, collections, opts);
        this[name] = replica;
        this.collections.push(replica);
        if (done) {
            done();
        }
    },
    ...
    configure = function (opts, callback) {
        ...
        var that = this,
            transStore = opts.transStore,
            dataModelDoc,
            confConn;

        if (!this.transaction && !transStore) {
            transStore = "_transactions";
        }

        this.env = opts.env || process.env.NODE_ENV || "development";
        // FUTURE: Add tests for custom config collection. [2014-01-12]
        if (opts.config) {
            ...
            case "object":
                confConn = opts.config.connection;
                this.addConnection(confConn.name, confConn.provider, confConn.options);
                confConn = confConn.name;
                break;
            default:
        ...
        async.series([
            ...
            function (done) {
                var type = typeof transStore,
                    transMan;

                if (type === "undefined") {
                    done();
                } else if (type === "string") {
                    that.addCollection(transStore, function (err) {
                        if (err) {
                            done(err);
                        }
                        that.transaction = new TransactionManager(that[transStore], that);
                        done();
                    });
                } else if (type === "object") {
                    if (transStore.insert && transStore.update && transStore.upsert && transStore.get && transStore.delete) {
                        transMan = new TransactionManager(transStore, that);
                    } else if (transStore.module) {
                        transMan = new TransactionManager(require(transStore.module)(transStore.options));
                    }

                    if (!transMan) {
                        return done(new Err("INVALID_TRANS_STORE"));
                    }

                    that.transaction = transMan;
                    done();
                } else {
                    done(new Err("INVALID_TRANS_STORE"));
                }
            },
            function (done) {
                if (!dataModelDoc || opts.omitModelDoc) {
                    return done();
                }
...
/**
 * Adds new data sotre connection for this instance.
 ...
 */
Manager.prototype.addReplicaSet = addReplicaSet;
