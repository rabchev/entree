...
/**
 * Merges the own properties of the source object to the destination object.
 *
 * This is utility method intended mainly for data provider implementers.
 *
 * @param {object} src - The source object.
 * @param {object} trg - The target object.
 * @return {object} - The the destination object with merged properties.
 */
...
Provider.prototype._setDefaultFields = function (item, context, insert) {
    if (insert) {
        if (this.schema.__createdAt__) {
            item[this.schema.__createdAt__] = new Date().toISOString();
        } else if (item._createdAt) {
            item._createdAt = new Date().toISOString();
        }

        if (context) {
            if (this.schema.__createdBy__) {
                item[this.schema.__createdBy__] = getUserIdentity(context);
            } else if (item._createdBy) {
                item._createdBy = getUserIdentity(context);
            }
        }

        if (this.schema.__version__) {
            item[this.schema.__version__] = 1;
        } else if (item._version) {
            item._version = 1;
        }
    } else {
        if (this.schema.__version__) {
            var fld = {};
            fld[this.schema.__version__] = 1;
            item.$inc = fld;
        } else if (item._version) {
            item.$inc = { _versoin: 1 };
        }
    ...
    if (this.schema.__modified__) {
        item[this.schema.__modified__] = new Date().toISOString();
    } else if (item._lastModified) {
        item._lastModified = new Date().toISOString();
    }

    if (context) {
        if (this.schema.__modifiedBy__) {
            item[this.schema.__modifiedBy__] = getUserIdentity(context);
        } else if (item._lastModifiedBy) {
            item._lastModifiedBy = getUserIdentity(context);
        }
    }
};
